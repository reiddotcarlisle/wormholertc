...
// sender.js - Magic Wormhole WebRTC Sender (Initiator) for wormhole-rtc
...
export async function senderGenerateAndConnect({ wordCount, onCode, onStep, onMessage, onConnected, onFailure }) {
    ...
    function connectToMailbox() {
        ...
        ws.onopen = async () => {
            ...
            // Always send blinded public key immediately after binding
            await sendBlindedPublicKey();
        };
    ...
    function initializePeerConnection() {
        ...
        peerConnection.onconnectionstatechange = () => {
            ...
            if (state === 'connected') {
                updateStep('senderStep3Status', 'success', 'WebRTC Connection Established!');
                appendMsg('You can now send messages via the Data Channel!', 'text-green-800 font-bold');
                // Do not call onConnected here
            } else if (state === 'failed' || state === 'disconnected') {
                updateStep('senderStep3Status', 'failure', `WebRTC connection failed: ${state}.`);
            } else if (state === 'new' || state === 'connecting') {
                updateStep('senderStep3Status', 'active', `WebRTC connection state: ${state}`);
            }
        };
    ...
    async function createWebRTCOffer() {
        if (!sessionKey || offerCreated) return;
        offerCreated = true;
        ...
        const payload = { sdp: peerConnection.localDescription.toJSON(), type: 'offer' };
        const encrypted = await crypto.encrypt(sessionKey, JSON.stringify(payload));
        ws.send(JSON.stringify({ type: 'add', phase: 'signal', body: encrypted }));
        appendMsg('Encrypted SDP Offer sent.', 'text-green-600');
        updateStep('senderStep2Status', 'active', 'SDP Offer sent. Waiting for answer...');
    }
    async function handleSignalMessage(encrypted) {
        if (!sessionKey) return;
        const decrypted = await crypto.decrypt(sessionKey, encrypted);
        const obj = JSON.parse(decrypted);
        if (obj.sdp && obj.type === 'answer') {
            ...
            updateStep('senderStep2Status', 'success', 'Received SDP answer. Waiting for ICE candidates...');
    ...
    // --- Main Sender Flow ---
    try {
        updateStep('senderStep1Status', 'active', 'Generating code and connecting...');
        wormholeCode = generateWormholeCode(wordCount);
        if (onCode) onCode(wormholeCode);
        nameplate = wormholeCode.split('-')[0];
        ...
        updateStep('senderStep1Status', 'failure', 'Step 1 failed: Could not initiate connection.');
    }
}
